...
class PaperSwingSettingsModal(discord.ui.Modal, title="🎮 [모의투자] 4h 듀얼 스윙 설정"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        logger.info(
            "AI 모의투자 스윙 설정 업데이트: user_id=%s enabled=True budget=%d weight=%d%% max_coins=%d",
            self._user_id, budget, weight, max_coins,
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"현재 가상 잔고: **{self._virtual_krw:,.0f} KRW**"
            ),
            inline=False,
        )
        embed.add_field(
...
class PaperScalpSettingsModal(discord.ui.Modal, title="🎮 [모의투자] 1h 스캘핑 설정"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        logger.info(
            "AI 모의투자 스캘핑 설정 업데이트: user_id=%s enabled=True budget=%d weight=%d%% max_coins=%d",
            self._user_id, budget, weight, max_coins,
        )
...
class PaperBothSettingsModal(discord.ui.Modal, title="🎮 [모의투자] 동시 가동 (스윙+스캘핑) 설정"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        embed.add_field(
            name="💰 현재 가상 잔고",
            value=f"**{self._virtual_krw:,.0f} KRW**",
            inline=True,
        )
        embed.add_field(
            name="📋 전략 요약",
