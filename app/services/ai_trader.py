...
※ 이 전략은 BTC 등 거시 시장 지수와 무관하게 개별 알트코인의 MA50 돌파 모멘텀만으로 진입을 판단한다.
※ 제공된 데이터는 이미 알트코인만 필터링된 목록이므로 BTC 거시 국면을 별도로 고려하지 말 것.

1. 진입 타점 — 개별 알트코인의 확인된 모멘텀 돌파 (Confirmed Momentum Breakout):
   ...
   - [최우선 진입] 4h RSI14 55~70 + 현재가 > 4h MA50 위에 있는 종목:
     → 이 조건이 충족된 알트코인은 시장 전반 분위기와 무관하게 '확인된 모멘텀 돌파'로 판단,
