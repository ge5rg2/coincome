...
6. 언행일치 규칙 (Self-Consistency — 출력 전 반드시 자체 검증):

   ▶ [매수 결정 시 요약 일치 의무]
   - picks 배열에 score ≥ 90인 종목이 1개라도 있다면, market_summary의 결론은
     절대 "관망이 최선", "진입 조건 미달", "관망을 선택한다" 등의 표현으로 끝나서는 안 된다.
   - 시장 전반이 혼조·약세이더라도 특정 종목에서 독립적인 강점이 포착된 경우,
     market_summary는 그 사실을 반영해야 한다.
   - 올바른 예: "전반적인 시장은 혼조세를 보이고 있으나, 일부 종목에서 독립적인 강한 모멘텀이
     포착되어 선별적으로 진입을 진행한다."
   - 잘못된 예: [요약에서 "전체 관망"] + [picks에 92점 종목 포함] → 이는 논리적 모순이므로
     반드시 수정 후 출력할 것.

   ▶ [리스크 언급 시 감점 의무]
   - reason 또는 market_summary에서 해당 종목에 대한 불안 요소를 조금이라도 인지·언급했다면
     (예: "RSI 과열 우려", "추가 하락 가능성 잔존", "불확실한 추세", "변동성 리스크" 등),
     그 종목의 score는 절대 90점 이상이 될 수 없다. 최대 85점 이하로 제한한다.
   - score 수치는 네가 작성한 reason 텍스트의 긍정/부정 온도와 정비례해야 한다.
     텍스트에 경고나 주의가 포함되었다면, score도 그 불확실성을 정직하게 반영할 것.

   ▶ [Self-Check — JSON 출력 직전 3단계 검증]
   단계 1: picks에 score ≥ 90이 있는가? → market_summary 결론이 "관망"으로 끝나는가?
           → YES(모순) → market_summary를 매수 액션과 일치하도록 재작성 후 출력.
   단계 2: reason 또는 market_summary에 부정적 표현이 있는가? → 해당 종목 score ≥ 90인가?
           → YES(모순) → score를 85 이하로 낮추거나(90 미만 → picks 자동 제외됨) 진입 포기.
   단계 3: picks가 비어 있거나 모든 종목의 score < 90인가?(= 실제 매수 없음)
           → market_summary에 "진입한다", "매수한다", "진입을 결정했다" 등의 표현이 있는가?
           → YES(모순) → 해당 표현을 반드시 "기술적 조건은 일부 충족했으나, 확신도(Score)가
             부족하여 최종적으로 관망을 결정했다"는 뉘앙스로 교체 후 출력.
   위 3단계를 모두 통과한 후에만 최종 JSON을 출력할 것.
"""
...
6. 언행일치 규칙 (Self-Consistency — 출력 전 반드시 자체 검증):
   ...
   ▶ [매수 결정 시 요약 일치 의무]
   - picks 배열에 score ≥ 90인 종목이 1개라도 있다면, market_summary의 결론은
     절대 "관망이 최선", "진입 조건 미달", "모멘텀 부재" 등의 표현으로 끝나서는 안 된다.
   - 시장 전반이 혼조·약세이더라도 특정 종목에서 독립적인 모멘텀이 포착된 경우,
     market_summary는 그 사실을 반드시 반영해야 한다.
   - 올바른 예: "전반적인 단기 흐름은 약세이나, 일부 종목에서 1h RSI와 거래량이 동반
     상승하는 독립적인 모멘텀이 포착되어 선별적으로 스캘핑 진입을 진행한다."
   - 잘못된 예: [요약에서 "전체 관망"] + [picks에 91점 종목 포함] → 논리적 모순이므로
     반드시 수정 후 출력할 것.

   ▶ [리스크 언급 시 감점 의무]
   - reason 또는 market_summary에서 해당 종목에 대한 불안 요소를 조금이라도 인지·언급했다면
     (예: "단기 과열 우려", "되돌림 가능성", "유동성 부족", "변동성 리스크" 등),
     그 종목의 score는 절대 90점 이상이 될 수 없다. 최대 85점 이하로 제한한다.
   - score 수치는 네가 작성한 reason 텍스트의 긍정/부정 온도와 정비례해야 한다.
     텍스트에 경고나 주의가 포함되었다면, score도 그 불확실성을 정직하게 반영할 것.
...
# ------------------------------------------------------------------
# MAJOR TREND 엔진 시스템 프롬프트 — 메이저 코인 추세 돌파 판별 (4h 봉)
# 대상: BTC·ETH·XRP·SOL·DOGE·ADA·SUI·PEPE 메이저 코인 전용
# 전제: 3중 기계적 필터(Close>EMA200 & EMA20>EMA50 & Close>BB_Upper) 통과 후 호출
# AI 역할: 거래량 수반 진짜 강세 돌파 vs 개미 터는 가짜 돌파(Fakeout) 판별
# TP 4.0% / SL 2.0% (손익비 2:1 — 모멘텀 지속을 길게 끌어가는 구조)
# ------------------------------------------------------------------

_CORE_MAJOR_TREND_PROMPT = """\
너는 4시간 봉 기반의 '메이저 코인 추세 돌파 판별가'야.
기계적 3중 필터(EMA200 장기 추세 + EMA20>EMA50 정배열 + BB상단 돌파)를 이미 통과한 메이저 코인에 대해
"이 돌파가 거래량을 수반한 진짜 강세 돌파인지, 개미를 터는 가짜 돌파(Fakeout)인지"를 판별한다.

분석 대상: BTC·ETH·XRP·SOL·DOGE·ADA·SUI·PEPE 등 메이저 코인 전용.
이 프롬프트에서 언급되지 않는 소형 알트코인은 절대 picks에 포함하지 말 것.
이미 유저가 보유 중인 코인은 반드시 제외해.
확신이 없으면 picks 배열을 비워서 관망해도 된다 — 관망 자체가 최고의 전략일 수 있다.

반드시 아래 JSON 형식으로만 응답해 (다른 텍스트 없음):
{
  "market_summary": "메이저 시장 분석 2~3문장.",
  "picks": [
    {
      "symbol":            "ETH/KRW",
      "score":             92,
      "reason":            "[메이저 트렌드] 4h 거래량 1.8배 급증 동반 BB상단 돌파, EMA 정배열 확인. TP 4.0% / SL 2.0%",
...
[핵심 판별 원칙 — 진짜 돌파 vs 가짜 돌파(Fakeout)]

1. 전제 조건 (이미 Python 레벨에서 검증 완료 — AI는 '질'과 '지속성'에 집중):
   - Close > EMA200: 장기 상승 추세 내 위치 (이미 확인됨)
   - EMA20 > EMA50: 단·중기 정배열 구조 (이미 확인됨)
   - Close > BB Upper(20봉, 2.0σ): 볼린저밴드 상단 돌파 (이미 확인됨)

2. 진짜 돌파 신호 (score 90 이상 부여 조건):
   - 4h 거래량이 최근 20봉 평균 대비 **1.5배 이상** 증가 (거래량 확증 필수)
   - BB 상단 돌파 후 종가가 BB 상단 위에서 마감 (종가 > BB 상단 = 강한 돌파)
   - 4h RSI가 50~75 구간 (모멘텀이 있되 과매수는 아닌 구간)
   - 이전 수평 저항선을 명확히 상향 돌파한 경우 추가 가산점

3. 가짜 돌파(Fakeout) 경고 신호 (하나라도 해당 시 score 90 미만 또는 관망):
   - 4h 거래량이 평소와 비슷하거나 오히려 감소 (거래량 미동반 돌파 = 가짜)
   - 4h RSI 75 초과 (단기 과매수 구간 — 되돌림 가능성 급증)
   - 직전 강한 수평 저항선 또는 전고점 바로 직전 (저항 직전 매도 압력)
   - 캔들 위꼬리가 몸통보다 길거나 도지 캔들로 마감 (매도 세력 등장 신호)
   - 최근 3~5봉 내 연속 급등 후의 돌파 (과열 신호 — 되돌림 위험)

4. 손절폭 / 목표가 (MAJOR 전용 하드 고정 — 절대 다른 값 사용 금지):
   - target_profit_pct: **4.0%** 고정 (추세 모멘텀 지속 기대, 짧게 먹지 말 것)
   - stop_loss_pct: **2.0%** 고정 (손익비 2:1 — 수익을 길게 끌기 위한 구조)
   ※ 이 값은 Python 코드에서 하드 상한으로 재검증됨 — 반드시 4.0 / 2.0 만 사용

5. 일반 규칙:
   - symbol은 "코인명/KRW" 형태 (예: BTC/KRW, ETH/KRW)
   - symbol은 반드시 "# MAJOR 코인 시장 데이터" 섹션에 존재하는 심볼만 사용
   - score 90 이상만 진입 (확실한 돌파만 — 관망을 두려워 말 것)
   - reason 필드는 반드시 "[메이저 트렌드]" 태그로 시작할 것
   - market_summary는 관망 시에도 반드시 작성 (관망 이유 포함)
   - market_summary와 reason에 분석 대상 외 소형 알트코인 언급 절대 금지
"""
...
class AITraderService:
    ...
    async def analyze_market(
        ...
        is_major = _engine == "MAJOR_TREND"

        if is_major:
            system_prompt = _CORE_MAJOR_TREND_PROMPT
        elif is_scalping:
        ...
        # ── 유저 프롬프트 구성 ────────────────────────────────────────
        # MAJOR_TREND 엔진: 블랙리스트 없음 (MAJOR 코인이 직접 분석 대상)
        # 별도 섹션 헤더로 AI에 컨텍스트 제공
        if is_major:
            lines: list[str] = ["# MAJOR 코인 시장 데이터 (4h 봉 기반 — 기계적 3중 필터 통과 종목만 포함)\n"]
            for symbol, data in market_data.items():
                price      = data.get("price")
                rsi4h      = data.get("rsi4h")
                vol_ratio  = data.get("vol_ratio")
                ema200     = data.get("ema200")
                ema20      = data.get("ema20")
                ema50      = data.get("ema50")
                bb_upper   = data.get("bb_upper")
                vol_krw    = data.get("volume_krw")

                lines.append(
                    f"- {symbol}: 현재가={format_krw_price(price)} KRW"
                    f" | 4h RSI={f'{rsi4h:.1f}' if rsi4h is not None else 'N/A'}"
                    f" | 거래량비율(최근/20봉평균)={f'{vol_ratio:.2f}x' if vol_ratio is not None else 'N/A'}"
                    f" | EMA200={format_krw_price(ema200)} | EMA20={format_krw_price(ema20)}"
                    f" | EMA50={format_krw_price(ema50)} | BB_Upper={format_krw_price(bb_upper)}"
                    f" | 24h거래대금={f'{vol_krw / 1e8:.1f}억' if vol_krw else 'N/A'}"
                )
            ...
            user_prompt = "\n".join(lines)
            logger.info(
                "[AI DEBUG - INPUT] MAJOR_TREND 프롬프트 (budget=%.0f):\n%s",
                available_krw, user_prompt,
            )

        else:
            # ── ALT 엔진: 블랙리스트 심볼 사전 로그 (AI에 데이터 미전달) ──
            ...
            # ── ALT 엔진: BTC 국면 컨텍스트 + 시장 데이터 프롬프트 구성 ──────
            # BTC 데이터를 주지 않으면서 "BTC RSI 기준으로 판단하라"고 지시하면
        ...
        for p in picks_raw:
            ...
            # MAJOR_TREND 엔진: 블랙리스트 = 타겟 유니버스 → 체크 스킵
            # ALT 엔진: Python 레벨에서 재확인 — 프롬프트 우회 원천 차단
            if not is_major and symbol in self._BLACKLIST:
                logger.warning(
            ...
            # MAJOR:    하드 상한 2.5% (TP 4.0% / SL 2.0% 기준, buffer)
            ...
            # MAJOR:    R:R ≥ 2.0, target 최소 4.0% (손익비 2:1 하드 고정)
            ...
            # MAJOR: target 4.0% 하한 추가 보장
            if is_major:
                min_target = max(min_target, 4.0)
