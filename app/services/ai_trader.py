...
- 매수할 종목이 없으면: {"market_summary": "...", "picks": []}
- [언행일치 규칙] market_summary에서 긍정적으로 분석했거나 "진입 유효", "매수 적합" 등으로 언급한 종목은 반드시(MUST) picks 배열에 매수 데이터로 포함해야 한다. 분석만 하고 picks를 비워두는 행동은 절대 금지한다.
- [공격적 실행 규칙] 1시간 봉 단타 모드에서는 관망(현금 대기)을 최소화하라. 전략 기준에 부합하는 종목이 단 1개라도 존재하면 주저하지 말고 즉시 picks에 담아 실행에 옮겨라.
"""
