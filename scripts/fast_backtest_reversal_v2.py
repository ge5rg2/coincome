"""
fast_backtest_reversal_v2.py — 낙폭과대 반등장 스나이핑 v2 (Tuned).

v1 (fast_backtest_reversal.py) 대비 파라미터만 조정한 미세 튜닝 버전.
LLM · 업비트 API 호출 없이 .cache/ohlcv/ JSON 파일만 읽어
순수 수학 연산으로 역추세 전략(Oversold Reversal)을 검증한다.

[전략 — 낙폭과대 반등장 스나이핑 v2 (Tuned): 타이트한 손절 + 더 깊은 과매도]
  진입: Close < MA50  AND  RSI < 25
        ─ MA50: 중장기 하락 추세 확인 (장기 하락 구간에 위치한 종목만 대상)
        ─ RSI < 25: v1(30)보다 더 깊은 극단적 과매도 — '패닉 셀(투매 소진)' 구간 집중
        ─ 진입 빈도 감소를 감수하고 더 높은 반등 확률 구간만 선별
  블랙리스트: BLACKLIST 등록 코인 자동 제외 (v1과 동일)
  익절: 진입가 대비 +3.0% 도달 (v1과 동일 — 기술적 반등 단기 수익 확보)
  손절: 진입가 대비 -2.5% 도달 (v1: 3.0% → v2: 2.5% — 칼날 낙하 조기 차단)
  우선순위: WIN > LOSS (동일 봉에서 TP·SL 동시 충족 시 WIN 적용)
  겹침 방지: 현재 포지션 청산 전까지 신규 진입 신호 무시 (per-symbol)

[v1 → v2 튜닝 목표]
  문제 인식: v1(RSI < 30 / SL 3.0%)은 진입이 너무 많아 '떨어지는 칼날'에
             베이는 깊이(MDD)가 과도, 손절 라인이 여유롭지 않음
  튜닝 방향:
    1. 손절 타이트화: SL 3.0% → 2.5%
       (추가 하락 시 더 빠른 손절로 MDD 축소 — 손실 단위 감소)
    2. 진입 조건 강화: RSI < 30 → RSI < 25
       (더 깊은 극단적 공포(패닉셀) 구간만 진입 — 반등 성공 확률 ↑, 진입 빈도 ↓)
  목표 지표: 승률 55% 이상, MDD 개선

[v1 vs v2 파라미터 비교]
  항목          │ v1 (Reversal)   │ v2 (Tuned)
  ─────────────────────────────────────────────
  TAKE_PROFIT   │ 3.0%            │ 3.0% (동일)
  STOP_LOSS     │ 3.0%            │ 2.5% ← 타이트화
  RSI_ENTRY_MAX │ 30.0            │ 25.0 ← 강화
  MA 조건       │ Close < MA50    │ Close < MA50 (동일)
  R:R           │ 1.0:1           │ 1.2:1 (TP 3.0% / SL 2.5%)

[의존성]
  표준 라이브러리만 사용 (pandas / requests 불필요)

실행 예시:
  python scripts/fast_backtest_reversal_v2.py
  python scripts/fast_backtest_reversal_v2.py --symbol LINK_KRW
  python scripts/fast_backtest_reversal_v2.py --tp 3.0 --sl 2.5 --rsi-max 25
  python scripts/fast_backtest_reversal_v2.py --timeframe 4h --csv
"""
...
# ──────────────────────────────────────────────────────────────────────────────
TAKE_PROFIT_PCT: float = 3.0   # 익절률 (%) — v1과 동일 (기술적 반등 단기 수익 확보)
STOP_LOSS_PCT:   float = 2.5   # 손절률 (%) — v1(3.0) → v2(2.5): 칼날 낙하 조기 차단

# v2: RSI 진입 임계를 30→25로 강화 — 더 깊은 극단적 공포 구간에서만 진입
# (진입 빈도 감소를 감수하고 반등 성공 확률이 높은 패닉셀 구간에 집중)
RSI_ENTRY_MIN:   float = 0.0   # 진입 허용 RSI 최솟값 (하한 없음 — 0.0으로 고정)
RSI_ENTRY_MAX:   float = 25.0  # 진입 허용 RSI 최댓값 (v1: 30.0 → v2: 25.0 강화)

MA50_PERIOD:     int   = 50    # 중장기 이동평균선 기간 (봉) — Close < MA50 하락 추세 확인
...
def calc_ma(closes: list[float], period: int = MA50_PERIOD) -> list[float | None]:
    """단순 이동평균(SMA)을 계산한다.
...
def load_ohlcv_files(
    ...
    # 최소 데이터 요건: MA50 워밍업 49봉 + RSI 워밍업 14봉 + 여유 5봉
    min_candles = MA50_PERIOD + RSI_PERIOD + 5

    result: dict[str, list[list]] = {}
    for path in files:
        # 심볼 추출: "LINK_KRW_4h.json" → "LINK_KRW"
...
def backtest_symbol(
    ...
    """단일 심볼 OHLCV 데이터로 낙폭과대 반등장 스나이핑 v2 전략 백테스트를 실행한다.

    [전략 진입 조건 — Oversold Reversal v2 (Tuned)]
      조건 A: 현재 종가 < MA50  (중장기 하락 추세 내 위치 확인 — 반등 타깃 구간)
      조건 B: RSI < RSI_ENTRY_MAX (25)  (패닉셀 극단 구간 — v1의 30보다 더 깊은 공포)
      → 두 조건 동시 충족 시 해당 봉 종가에 시장가 매수 (기술적 반등 기대)

    [v1 대비 변경 사항]
      손절: 3.0% → 2.5%  (칼날 낙하 조기 차단, MDD 축소 목표)
      RSI : 30   → 25    (더 깊은 극단적 공포 구간에만 진입, 승률 ↑)
      R:R : 1.0:1 → 1.2:1 (TP 3.0% / SL 2.5%)
    ...
    상방 이동이 먼저 발생한다고 가정해 WIN 우선 적용.

    Args:
        sym:    심볼 식별자 (예: "LINK_KRW")
        ohlcv:  [[ts_ms, open, high, low, close, volume], ...] (오름차순)
        tp_pct: 익절률 (%) — 기본 3.0%
        sl_pct: 손절률 (%) — 기본 2.5% (v1: 3.0%)
        rsi_min: 진입 허용 RSI 최솟값 (기본 0.0 — 사실상 하한 없음)
        rsi_max: 진입 허용 RSI 최댓값 (기본 25.0 — v1: 30.0)
    ...
    # 역추세 전략: MA50 (하락 추세 확인) + RSI (극단적 과매도 확인)
    # MA20은 역추세 진입 로직에 불필요하므로 계산 제외
    ma50_vals = calc_ma(closes, MA50_PERIOD)
    ...
    for i in range(1, len(ohlcv)):
        ...
        if in_position:
            ...
            # 우선순위 2: LOSS — SL 도달 (추가 하락 — 조기 손절로 MDD 차단)
            if l <= sl_price:
                trades.append({
                    "symbol":       sym,
                    "entry_ts":     entry_ts_val,
        ...
        # ── [미보유] 진입 조건 확인 ──────────────────────────────────────────
        ma50_curr = ma50_vals[i]
        rsi_curr  = rsi_vals[i]

        # 지표 미준비 구간 (워밍업) → 스킵
        # MA50 워밍업(49봉)이 가장 길어 이 구간이 사실상 지배
        if ma50_curr is None or rsi_curr is None:
            continue

        close_curr = closes[i]

        # 조건 A: 현재 종가 < MA50  (중장기 하락 추세 내 위치 — 반등 타깃 구간)
        # 목적: 상승 추세 내 과매도는 추세 강화 신호일 수 있으므로, 하락 추세만 타깃
        below_ma50 = close_curr < ma50_curr

        # 조건 B: RSI < RSI_ENTRY_MAX (25)  (패닉셀 극단 구간 — v1의 30보다 더 깊은 공포)
        # rsi_min = 0.0 으로 사실상 단일 상한 조건 (RSI < 25)
        rsi_ok = rsi_min <= rsi_curr <= rsi_max

        if below_ma50 and rsi_ok:
            # 진입: 해당 봉 종가에 매수 체결 (기술적 반등 기대)
            in_position  = True
...
def save_csv(trades: list[dict], tp_pct: float, sl_pct: float) -> Path:
    ...
    out_path = RESULT_DIR / f"fast_backtest_reversal_v2_tp{tp_pct:.0f}_sl{sl_pct:.1f}_{ts_str}.csv"
...
def print_summary(
    ...
    Args:
        ...
        tp_pct:          익절률 (%).
        sl_pct:          손절률 (%).
        rsi_min:         진입 RSI 최솟값 (표기용).
        rsi_max:         진입 RSI 최댓값 (표기용).
    ...
    print(f"\n{_C}{_B}{'=' * 64}{_RS}")
    print(f"{_C}{_B}  📉 FAST BACKTEST  —  Oversold Reversal v2 (Tuned){label}{_RS}")
    print(f"{_C}{sep2}{_RS}")
    print(f"  타임프레임  : {timeframe}봉")
    print(f"  진입 파라미터: MA{MA50_PERIOD} (하락 추세 확인)  |  RSI < {rsi_max:.0f} (패닉셀 극단 과매도)")
    ...
    if total == 0:
        print(f"\n{_Y}  [경고] 체결된 거래 없음.{_RS}")
        print(f"  진입 조건(RSI < {rsi_max:.0f}, Close < MA{MA50_PERIOD})을")
        print(f"  만족하는 구간이 현재 캐시 데이터에 없습니다.")
        print(f"  RSI < {rsi_max:.0f}는 매우 드문 극단 구간입니다.")
        print(f"  --rsi-max 를 높이거나 (예: 30) 캐시 봉 수를 늘려보세요.\n")
        return
    ...
    # ── 핵심 지표 출력 ─────────────────────────────────────────────────────
    # v2 목표: 승률 55% 이상 (R:R 1.2:1이므로 손익분기 45% 이상이나 목표는 55%)
    wc = _G if win_rate >= 55 else (_Y if win_rate >= 45 else _R)
    win_label = (
        "✅ 목표 달성 (≥55%)" if win_rate >= 55
        else ("⚠️ 손익분기 이상 (<55%)" if win_rate >= 45
              else "❌ 손실 구조 (<45%)")
    )
    ...
    print(f"  {'승률         :':<22} {wc}{win_rate:.1f}%{_RS}  {win_label}")
...
def print_comparison(
    ...
    s_mdd_delta = f_s_mdd - c_s_mdd
    s_mdd_c     = _G if s_mdd_delta >= 0 else _R
    s_mdd_arrow = "▼" if s_mdd_delta >= 0 else "▲"
    print(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "로컬 OHLCV 캐시 기반 초고속 전략 백테스트 "
            "(Oversold Reversal v2 — RSI < 25 패닉셀 스나이핑, SL 2.5% 타이트화)"
        ),
    ...
    parser.add_argument(
        ...
        help="익절률 (%%) — 기술적 반등 목표 (기본 3.0%%)",
    )
    parser.add_argument(
        "--sl",
        type=float,
        default=STOP_LOSS_PCT,
        help="손절률 (%%) — 칼날 낙하 조기 차단 (기본 2.5%%, v1: 3.0%%)",
    )
    parser.add_argument(
        "--rsi-min",
        type=float,
        default=RSI_ENTRY_MIN,
        help="진입 허용 RSI 최솟값 (기본 0.0 — 하한 없음)",
    )
    parser.add_argument(
        "--rsi-max",
        type=float,
        default=RSI_ENTRY_MAX,
        help="진입 허용 RSI 최댓값 (기본 25.0 — 패닉셀 극단 구간, v1: 30.0)",
    )
    parser.add_argument(
        "--csv",
        action="store_true",
    ...
    logger.info(
        "[Oversold Reversal v2 (Tuned)] TP +%.1f%%  SL -%.1f%%  RSI < %.0f  MA%d (하락 추세 확인)  블랙리스트 %d개",
        tp_pct, sl_pct, rsi_max, MA50_PERIOD, len(found_blacklist),
    )
