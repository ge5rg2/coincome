"""
fast_backtest_reversal.py — 로컬 OHLCV 캐시 기반 초고속 역추세 전략 백테스트.

LLM · 업비트 API 호출 없이 .cache/ohlcv/ JSON 파일만 읽어
순수 수학 연산으로 낙폭과대 반등장 스나이핑(Oversold Reversal) 전략을 검증한다.

[전략 — 낙폭과대 반등장 스나이핑 (Oversold Reversal v1): 메이저 코인 블랙리스트 필터]
  진입: Close < MA50  AND  RSI < 30
        ─ MA50: 중장기 하락 추세 확인 (장기 하락 구간에 위치한 종목만 대상)
        ─ RSI < 30: 극단적 과매도 상태 — 투매 소진 후 기술적 반등 타점 포착
        ─ MA20 조건 미적용: 역추세 특성상 단기 추세선 돌파 요건 불필요
  블랙리스트: BLACKLIST 등록 코인 자동 제외
              (BTC/ETH/XRP 등 구조적 하락이 아닌 횡보 특성 메이저 코인 배제)
  익절: 진입가 대비 +TAKE_PROFIT_PCT% 도달 (기본 +3.0% — 기술적 반등 단기 수익 확보)
  손절: 진입가 대비 -STOP_LOSS_PCT%  도달 (기본 -3.0% — 추가 하락(지하실) 방어)
  우선순위: WIN > LOSS (동일 봉에서 TP·SL 동시 충족 시 WIN 적용)
  겹침 방지: 현재 포지션 청산 전까지 신규 진입 신호 무시 (per-symbol)

[v1 전략 설계 근거]
  타겟 장세: 하락장/폭락장 — 모멘텀 돌파 전략(v7)이 관망하는 구간을 역으로 공략
  핵심 가설: 중장기 하락 추세(Close < MA50) 내에서 RSI < 30 극단적 투매가 나온 직후
             단기 기술적 반등(dead cat bounce)이 발생하며 짧은 수익 구간 존재
  손익비   : TP 3.0% / SL 3.0% → R:R 1.0:1 (대칭 구조)
             → 역추세 특성상 방향 예측 난이도 높음: 높은 승률이 수익성의 핵심

[fast_backtest.py (Momentum Breakout v7) 대비 변경 사항]
  TAKE_PROFIT_PCT : 6.0 → 3.0  (짧은 반등만 확보)
  STOP_LOSS_PCT   : 4.0 → 3.0  (추가 하락 빠른 손절)
  RSI_ENTRY_MAX   : 70.0 → 30.0 (과매도 진입 — RSI < 30)
  RSI_ENTRY_MIN   : 55.0 → 0.0  (하한 없음 — 사실상 RSI < 30 단일 조건)
  진입 조건       : Close > MA20 & MA50 → Close < MA50 (하락 추세 확인)

[의존성]
  표준 라이브러리만 사용 (pandas / requests 불필요)

실행 예시:
  python scripts/fast_backtest_reversal.py
  python scripts/fast_backtest_reversal.py --symbol ETH_KRW
  python scripts/fast_backtest_reversal.py --tp 3.0 --sl 3.0
  python scripts/fast_backtest_reversal.py --timeframe 4h --csv
  python scripts/fast_backtest_reversal.py --symbol ETH_KRW --tp 3 --sl 3 --csv
"""
...
# ──────────────────────────────────────────────────────────────────────────────
TAKE_PROFIT_PCT: float = 3.0   # 익절률 (%) — 진입가 대비 이 % 도달 시 전량 익절 (기술적 반등 단기 확보)
STOP_LOSS_PCT:   float = 3.0   # 손절률 (%) — 진입가 대비 이 % 하락 시 전량 손절 (추가 하락 방어)

# 역추세 전략: RSI < RSI_ENTRY_MAX 단일 조건 (극단적 과매도 진입)
# RSI_ENTRY_MIN은 사실상 0.0 — "RSI < 30" 단일 조건과 동일
RSI_ENTRY_MIN:   float = 0.0   # 진입 허용 RSI 최솟값 (하한 없음 — 0.0으로 고정)
RSI_ENTRY_MAX:   float = 30.0  # 진입 허용 RSI 최댓값 (극단적 과매도 임계: RSI < 30)
...
# 메이저 코인 블랙리스트 — fast_backtest.py와 동일하게 유지
# (모멘텀 전략과 달리 역추세는 메이저 코인이 오히려 반등이 안정적일 수 있으나,
#  비교 기준 통일을 위해 동일 블랙리스트 적용)
BLACKLIST: list[str] = [
...
def load_ohlcv_files(
    ...
    Args:
        ...
        symbol_filter: 특정 심볼만 로드 (예: "ETH_KRW"). None 이면 전체 로드.

    Returns:
        {"ETH_KRW": [[ts, o, h, l, c, v], ...], ...}
    """
...
def backtest_symbol(
    ...
    """단일 심볼 OHLCV 데이터로 낙폭과대 반등장 스나이핑 전략 백테스트를 실행한다.

    [전략 진입 조건 — Oversold Reversal v1]
      조건 A: 현재 종가 < MA50  (중장기 하락 추세 내 위치 확인 — 반등 타깃 구간)
      조건 B: RSI < RSI_ENTRY_MAX (30)  (극단적 과매도 투매 소진 신호)
    ...
    Args:
        ...
        rsi_max: 진입 허용 RSI 최댓값 (기본 30.0 — 극단적 과매도 기준)
    ...
    # 역추세 전략: MA50 (하락 추세 확인) + RSI (과매도 확인)
    ...
    for i in range(1, len(ohlcv)):
        ...
        # 조건 B: RSI < RSI_ENTRY_MAX (30)  (극단적 과매도 투매 소진 구간)
        # rsi_min = 0.0 으로 사실상 단일 상한 조건 (RSI < 30)
...
def save_csv(trades: list[dict], tp_pct: float, sl_pct: float) -> Path:
    ...
    out_path = RESULT_DIR / f"fast_backtest_reversal_tp{tp_pct:.0f}_sl{sl_pct:.0f}_{ts_str}.csv"
...
def print_summary(
    ...
    print(f"\n{_C}{_B}{'=' * 64}{_RS}")
    print(f"{_C}{_B}  📉 FAST BACKTEST  —  Oversold Reversal v1{label}{_RS}")
    print(f"{_C}{sep2}{_RS}")
    print(f"  타임프레임  : {timeframe}봉")
    print(f"  진입 파라미터: MA{MA50_PERIOD} (하락 추세 확인)  |  RSI < {rsi_max:.0f} (극단적 과매도)")
    ...
    if total == 0:
        ...
        print(f"  --rsi-max 를 높이거나 (예: 35) 캐시 봉 수를 늘려보세요.\n")
        return
    ...
    # ── 핵심 지표 출력 ─────────────────────────────────────────────────────
    # 역추세 전략은 R:R이 1:1이므로 승률 50% 이상이 손익분기
    # 목표 승률 기준을 50%로 상향 적용
    wc = _G if win_rate >= 50 else _R
    ...
    print(
        ...
        f"  {'✅ 수익 구조 (≥50%)' if win_rate >= 50 else '❌ 손실 구조 (<50%)'}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="로컬 OHLCV 캐시 기반 초고속 전략 백테스트 (Oversold Reversal — 낙폭과대 반등장 스나이핑)",
    ...
    parser.add_argument(
        ...
        help="진입 허용 RSI 최댓값 (기본 30.0 — 극단적 과매도 기준)",
    )
    parser.add_argument(
        "--csv",
        action="store_true",
    ...
    logger.info(
        "[Oversold Reversal v1] TP +%.1f%%  SL -%.1f%%  RSI < %.0f  MA%d (하락 추세 확인)  블랙리스트 %d개",
