"""
One-time DB migration: users 테이블에 AI 자동 매매 관련 컬럼 2개 추가.

추가 컬럼:
  - ai_mode_enabled  BOOLEAN NOT NULL DEFAULT FALSE
  - ai_trade_amount  INTEGER NOT NULL DEFAULT 10000

실행 방법:
  python -m scripts.add_ai_columns
...
_COLUMNS: list[tuple[str, str]] = [
    (
        "ai_mode_enabled",
        "ALTER TABLE users ADD COLUMN ai_mode_enabled BOOLEAN NOT NULL DEFAULT FALSE",
    ),
    (
        "ai_trade_amount",
        "ALTER TABLE users ADD COLUMN ai_trade_amount INTEGER NOT NULL DEFAULT 10000",
    ),
]
