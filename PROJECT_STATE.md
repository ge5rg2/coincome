...
매시 정각 실행 (00:00~23:00, 24회/일)
  ...
  ③ BTC 4h EMA50 Regime 판별 (SWING 또는 SCALPING 엔진 실행 시):
     - _fetch_btc_regime(): BTC/KRW 4h 캔들 60봉 조회 → EMA50 계산
     - 현재가 > EMA50 → BULL / 이하 → BEAR (실패 시 기본값 BULL)
     - BEAR 시 ai_trader analyze_market 시스템 프롬프트에 방어 지시사항 동적 주입:
       SCALPING BEAR: 매력도 85점 상한 + TP 1.5% 이하 단기 목표 강제
       SWING BEAR: 전략A(추세 돌파) 전면 금지 + 전략B(RSI<25 낙폭반등)만 허용
  ④ SWING 엔진 (is_swing_hour=True 시):
     - 예산: min(KRW 잔고, ai_swing_budget_krw - swing_invested)
     - analyze_market(engine_type="SWING", weight_pct=ai_swing_weight_pct, regime=_btc_regime)
     - score≥90 픽 → _buy_new_coins(engine_type="SWING")
  ⑤ SCALPING 엔진 (매 사이클):
     - 예산: min(KRW 잔고, ai_scalp_budget_krw - scalp_invested)
     - analyze_market(engine_type="SCALPING", weight_pct=ai_scalp_weight_pct, regime=_btc_regime)
  ...
  ⑩ DM 리포트 전송 (엔진별 섹션 통합 임베드, engine_tag 정확 표시, ManualSellView 미첨부)
     수동 청산은 /내포지션 커맨드 전용 — 정기 리포트에서 View 제거 (2026-03-29)
```
...
| 등급     | 최대 코인 수 | 최대 1회 투자   | max_active_engines | AI 실전 모드                          | AI 모의 모드              |
| -------- | ------------ | --------------- | ------------------ | ------------------------------------- | ------------------------- |
| **FREE** | 2개          | 100,000 KRW     | 0 (AI 불가)        | ✗ (차단)                              | ✗ (차단)                  |
| **PRO**  | 999개        | 100,000,000 KRW | 1 (알트 1개)       | ✓ 알트 스윙 or 알트 스캘핑 중 1개    | ✓ 알트 스윙 or 알트 스캘핑 중 1개 |
| **VIP**  | 999개        | 100,000,000 KRW | 3 (전체 엔진)      | ✓ 알트 스윙 + 알트 스캘핑 + 메이저    | ✓ 동일 (복수 선택 가능)   |

> - `max_active_engines` 컬럼 (2026-03-30 추가): DB 기반 등급 제한 제어
> - AI 트레이딩 UI: `/ai실전`, `/ai모의` 모두 `max_active_engines==0` 시 즉시 차단
> - PRO UI: 버튼 방식 1개 택 1 (알트 스윙 / 알트 스캘핑 / OFF), MAJOR 미제공
> - VIP UI: 토글 버튼 복수 선택 (최대 3엔진) + 다음 버튼으로 동적 Modal(3~5필드) 진행
> - 예산 범위: 실전 50,000~10,000,000 KRW / 모의 500,000~10,000,000 KRW (이전: 1,000,000~100,000,000)
...
| **등급 엔진 제한 필드**                 |                 |            | **2026-03-30 추가**                                        |
| `max_active_engines`                    | Integer         | 1          | 등급별 활성 가능 엔진 수 (FREE=0, PRO=1, VIP=3)            |
...
| `ADMIN_API_KEY`                   | Admin Dashboard API 인증 키 (X-Admin-API-Key 헤더, 운영 시 강력한 난수값 필수) |
