...
> **기준일**: 2026-03-18 (최종 수정: 2026-03-18 — fast_backtest.py v1 결과 분석 + MA50 추세 필터 v6 전략 준비)
> **현재 작업 브랜치**: `backtest`
...
| 우선순위 | 항목 | 관련 브랜치 |
|---|---|---|
| 🔴 높음 | **fast_backtest.py v6 검증** — MA50 추세 필터 + RSI 55~70 + TP 6%/SL 4% 파라미터로 재백테스트 | `backtest` |
| 🔴 높음 | **가짜 돌파(휩쏘) 제거 확인** — 메이저 코인(BTC/ETH/XRP/DOGE) 승률 40%+ 달성 목표 | `backtest` |
| 🟡 보통 | **PR #35 리뷰·병합** — 백테스터+fast_backtest를 `dev`에 통합 | `backtest` → `dev` |
...
#### ✅ v6 전략 개선 방향 — MA50 추세 필터 도입 (2026-03-18)

**변경 파라미터**: 4h봉 / Close>MA20 **AND Close>MA50** / RSI **55~70** / TP +**6.0**% / SL -4.0% (R:R **1.5:1**)

| 항목 | 변경 전 (v1) | 변경 후 (v6) | 목적 |
|---|---|---|---|
| 추세 필터 | Close > MA20만 | **Close > MA20 AND Close > MA50** | MA50 장기 상승 추세 내 진입만 허용 → 휩쏘 방어 |
| RSI 진입 하한 | 50.0 | **55.0** | 모멘텀 "확인" 구간으로 상향 (50~54는 중립 제외) |
| RSI 진입 상한 | 65.0 | **70.0** | 상승 모멘텀 유효 구간 확장 (+5 포인트) |
| TP (익절률) | +8.0% | **+6.0%** | 목표 도달 확률 향상 → 실질 승률 개선 |
| SL (손절률) | -4.0% | -4.0% (유지) | 유지 |
| R:R | 2.0:1 | **1.5:1** | 타이트한 목표로 승률 우선 전략 |

**v6 진입 조건 (3개 AND)**:
```
조건 A: Close > MA20   (단기 상승 돌파 확인)
조건 B: Close > MA50   (중장기 상승 추세 내 위치 확인) ← NEW
조건 C: RSI 55~70      (강한 모멘텀 초입, 과매수 진입 금지)
```
