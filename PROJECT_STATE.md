...
> **기준일**: 2026-03-22 (최종 수정: 2026-03-22 — 역추세 전략 v1/v2 백테스트 추가 · Phase 7 하락장 대응 전략 검증 시작)
> **현재 작업 브랜치**: `backtest`
...
| 우선순위 | 항목 | 관련 브랜치 |
|---|---|---|
| 🔴 높음 | **Paper Trading(모의투자) 모드 가동** — `/ai모의` 커맨드로 v7 전략 실시간 Forward Testing 시작 | `dev` |
| 🔴 높음 | **Forward Testing 검증** — 실시간 모의투자 승률·MDD·ROI가 백테스트(승률 44.6% / MDD -19.1%) 수준 재현 확인 | `dev` |
| 🟡 보통 | **`ANTHROPIC_API_KEY` .env 등록** — `/ai실전` · `/ai모의` 운영 가동 전제 조건 | `dev` |
| 🟡 보통 | **PR #35 리뷰·병합** — backtest 브랜치(fast_backtest + ai_trader v7) → `dev` 통합 | `backtest` → `dev` |
...
### 📉 Phase 7 — 하락장 대응 역추세 전략 검증 (2026-03-22 ~ )
...
|---|---|---|
| **진입 조건** | Close > MA50 (상승 추세) + RSI 55~70 | Close < MA50 (하락 추세) + RSI < 30 |
| **목표** | 상승 모멘텀 지속 수익 | 기술적 반등 단기 수익 |
| **장세 적합성** | 상승장 / 강세 모멘텀 장 | 하락장 / 급락 후 공황 저점 구간 |
| **전략 철학** | 추세 추종 | 역추세 (Contrarian) |

두 전략이 **[상승장 = 모멘텀 돌파 / 하락장 = 낙폭과대 스나이핑]** 쌍으로 작동하여
전천후 엔진 통합을 최종 목표로 설정.
...
**진입 조건 (fast_backtest.py 대비 변경)**:
```python
# 추세 돌파 v7 (기존)
above_ma20 = close_curr > ma20_curr
above_ma50 = close_curr > ma50_curr
rsi_ok = 55.0 <= rsi_curr <= 70.0

# 역추세 Reversal v1 (신규)
below_ma50 = close_curr < ma50_curr   # 하락 추세 확인
rsi_ok = 0.0 <= rsi_curr <= 30.0     # 극단적 과매도 구간
```

**승률 기준**: ≥50% (R:R 1:1 손익분기)
...
**분석**: 승률 50% 초과로 통계적 Edge는 확인됨. 단, 하락장 특성상 손절이 특정 기간에 집중되어
MDD가 지나치게 높음. 실전 투입 전 방어력 개선 필수.
...
**테스트 진행 중** — 타이트한 손절(-2.5%)과 더 깊은 공포(RSI 25) 구간 진입으로
MDD 억제 + 승률 55% 이상 확보 시도.

#### 최종 통합 목표 (Engine Integration)

v2 백테스트에서 방어력이 입증되면, `ai_trader.py`의 메인 프롬프트에 역추세 조건을 추가하여
**[상승장 = 모멘텀 돌파 / 하락장 = 낙폭과대 스나이핑]** 을 동시에 수행하는
전천후 엔진으로 통합 예정.

```
[현재 ai_trader.py 진입 조건]
  Close > MA50 (상승 추세)
  RSI 55~70 (모멘텀 확인)
  → 하락장에서 진입 0회 → 자금 회전율 급락

[통합 예정 이중 엔진]
  ① 모멘텀 돌파 (기존): Close > MA50 + RSI 55~70 → 상승장 수익
  ② 역추세 스나이핑 (신규): Close < MA50 + RSI < 25 → 하락장 수익
