...
| 우선순위 | 항목 | 관련 브랜치 |
|---|---|---|
| 🔴 높음 | **fast_backtest.py v7 검증** — 블랙리스트 8개 코인 제외, MDD 개선 효과 수치 확인 | `backtest` |
| 🔴 높음 | **BEAST MDD -50% 이하 달성 목표** — 현재 -88.6%, 블랙리스트 적용 후 목표치 확인 | `backtest` |
| 🟡 보통 | **AI 트레이더 프롬프트 이식 준비** — 검증된 퀀트 전략(MA50 추세 필터, RSI 55~70) 프롬프트 반영 | `backtest` |
...
#### 🆕 v7 전략 개선 방향 — 메이저 코인 블랙리스트 (2026-03-18)

**전략**: v6 파라미터 전면 유지 + BLACKLIST 8개 코인 진입 원천 차단 (알트코인 집중)

| 블랙리스트 코인 | 제외 이유 |
|---|---|
| BTC_KRW | 대형 매물대 휩쏘 빈발 — v6 승률 ~15% |
| ETH_KRW | 메이저 코인 공통 취약 — v6 승률 ~18% |
...
**v7 기대 효과**: 알트코인 집중 전략으로 BEAST MDD -88.6% → 목표 -50% 이하
