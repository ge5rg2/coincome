...
> **기준일**: 2026-03-18 (최종 수정: 2026-03-18 — PROJECT_STATE 전면 동기화 · 오기입 수정 · 개발 여정 정리)
> **현재 작업 브랜치**: `dev`
> **최신 안정 브랜치**: `dev` (커밋 `7dc13ce`)
...
매시 정각 실행 (00:00~23:00, 24회/일)
  SNIPER(=SWING)  유저: 01·05·09·13·17·21시 KST에만 처리 (6회/일, 4h 봉 기준)
  BEAST(=SCALPING) 유저: 매시 정각 처리 (24회/일, 1h 봉 기준)

  ① 연착륙 체크 (ai_is_shutting_down → 신규 매수 차단)
  ② 실전 가용 예산 계산 (ai_budget_krw > 0이면 예산 기반, 0이면 무제한)
  ③ review_existing_positions() → HOLD / UPDATE / SELL 결정
  ④ _buy_new_coins() → analyze_market() → score≥90 픽 선택
...
| `ai_mode_enabled` | Boolean | False | AI 실전 매매 ON/OFF (VIP 전용) |
...
| `ai_trade_style` | String(20) | `"SWING"` | SNIPER/BEAST 저장 (하위 호환: SWING/SCALPING) |
| `virtual_krw` | Float | 10,000,000 | 모의투자 가상 KRW 잔고 |
| `ai_budget_krw` | Float | 0.0 | AI 운용 예산 한도 (0=무제한) |
...
| `trade_style` | String(20) | AI 매수 당시 전략 (SNIPER/BEAST, 하위 호환: SWING/SCALPING) |
...
└── scripts/
    ...
    └── fast_backtest.py             # 로컬 캐시 기반 초고속 퀀트 전략 백테스트 (backtest 브랜치)
```
...
| 우선순위 | 항목 | 관련 브랜치 |
|---|---|---|
| 🔴 높음 | **Forward Testing 실시간 검증** — SNIPER/BEAST 모의투자 가동 후 승률·MDD·ROI가 백테스트(승률 44.6% / BEAST ROI +91.4% / MDD -19.1%) 수준 재현 확인 | `dev` |
| 🔴 높음 | **`ANTHROPIC_API_KEY` .env 등록** — `/ai실전` · `/ai모의` 운영 가동 전제 조건 | `dev` |
| 🟡 보통 | **`dev` → `main` PR 생성** — 현재 dev 커밋(`7dc13ce`) 기준 운영 배포 | `dev` → `main` |
| 🟡 보통 | AI 매매 성과 리포트 (실전 이력 집계 → Discord DM, trade_history 테이블 활용) | 신규 브랜치 필요 |
| 🟡 보통 | 프롬프트 추가 검증 — BEAST 모드 진입 공격성 및 관망 재발 여부 모니터링 | `dev` |
...
**수정 후 `_CORE_SNIPER_PROMPT` 핵심 변경점**:
```
[기존] 1. BTC 시장 국면 필터 (최우선):
         - BTC RSI < 45 → picks 완전히 비울 것
         - BTC RSI 45~55 → score 95+ 만 고려, 아니면 관망
         → AI가 BTC 데이터 없이도 수치를 지어내 관망하는 환각 버그 발생

[수정] ※ 이 전략은 BTC 등 거시 시장 지수와 무관하게
       개별 알트코인의 MA50 돌파 모멘텀만으로 진입을 판단한다.
       ※ 제공된 데이터는 이미 알트코인만 필터링된 목록이므로
       BTC 거시 국면을 별도로 고려하지 말 것.
   1. 진입 타점 — 개별 알트코인의 확인된 모멘텀 돌파:
      → 조건 충족 알트코인은 시장 전반과 무관하게 공격적으로 픽할 것
```
...
> 2026-03-12 착수 → 2026-03-18 현재 기준. git log 전수 조회(~100개) 결과를 6단계로 정리.
...
**ai_trader.py v7 이식 — 3중 방어 구조**:
```
[1단계] Python 블랙리스트: 8개 심볼 데이터 AI에 미전달 (원천 차단)
[2단계] 프롬프트 룰:      RSI 55+, Close>MA50, score≥90 절대 조건 명시
[3단계] 코드 검증:        SL > 5% 스킵 / R:R < 1.5 자동 보정 (파이썬 안전망)
```
...
### 현재 시스템 상태 (2026-03-18 기준)

```
✅ 완료된 것들
  - AI 실전 매매 (VIP): claude-sonnet-4-6 기반 SNIPER/BEAST 듀얼 모드
  - AI 모의투자 (전 등급): 동일 v7 전략, 가상 KRW 잔고 시뮬레이션
...
🎯 다음 단계 (Forward Testing)
  ...
  - 백테스트(승률 44.6% / BEAST ROI +91.4% / MDD -19.1%) 재현 확인
